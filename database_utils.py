...
class DatabaseUtils:
    ...
    def executar(self, statement, valores=()):
    ...
    def inserir_acordao_se_nao_existir(self, acordao):
        self.executar("INSERT INTO acordao (nome) VALUES (%s) ON CONFLICT (nome) DO NOTHING;", (acordao,))

    def inserir_ementa_se_nao_existir(self, ementa):
        self.executar("INSERT INTO ementa (nome) VALUES (%s) ON CONFLICT (nome) DO NOTHING;", (ementa,))

    def inserir_ou_atualizar_conteudo(self, acordao, conteudo):
        self.executar("""INSERT 
    ...
    def exportar_arquivo(self, caminho):
        self.executar("""Copy (select a.nome_acordao, regexp_replace(a.conteudo, E'[\\n\\r]+', ' ', 'g' )
